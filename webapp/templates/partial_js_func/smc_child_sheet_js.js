...
editSmcForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    showDiv("hidechildtable");
    const formData = new FormData(editSmcForm);
    const payload = {
            idcampaign: parseInt(document.getElementById("campaign").value, 10),
            idheader: parseInt(document.getElementById("aic").value.replace(/^SMC-/, ""), 10)
        };
    try {
        const response = await fetch(`${window.API_BASE_URL}/campaign/read/smc-aic-child-stats`, {
            method: "POST",
            headers: {
        ...
        if (result.status === "success" && result.data.length > 0) {
            ...
            result.data.forEach((child) => {
                ...
                row.innerHTML = `
                    <td class="px-3 py-2">${child.pname}</td>
                    <td class="px-3 py-2">${child.divname}</td>
                    <td class="px-3 py-2">${child.districtname}</td>
                    <td class="px-3 py-2">${child.tehsilname}</td>
                    <td class="px-3 py-2">${child.ucname}</td>
                    <td class="px-3 py-2">${child.aicid}</td>
                    <td class="px-3 py-2">${child.id_team}</td>
                    <td class="px-3 py-2">${child.supervisor_name || "-"}</td>
                    <td class="px-3 py-2">${child.campaign_name}</td>
                    <td class="px-3 py-2">${child.aicheader}</td>
                    <td class="px-3 py-2">${child.day_number}</td>
                    <td class="px-3 py-2">${child.house_number}</td>
                    <td class="px-3 py-2">${child.child_name}</td>
                    <td class="px-3 py-2">${child.gender}</td>
                    <td class="px-3 py-2">${child.age}</td>
                    <td class="px-3 py-2">${child.father_name}</td>
                    <td class="px-3 py-2">${child.address}</td>
                    <td class="px-3 py-2">${child.no_of_miss_round}</td>
                    <td class="px-3 py-2">${child.missedreason || "-"}</td>
                    <td class="px-3 py-2">${child.subreason || "-"}</td>
                    <td class="px-3 py-2">${child.location}</td>
                    <td class="px-3 py-2">${child.hrmp}</td>
                    <td class="px-3 py-2">${child.return_data || "-"}</td>
                    <td class="px-3 py-2">${child.data_vaccination || "-"}</td>
                    <td class="px-3 py-2">${child.username}</td>
                `;
            ...
            tableBody.innerHTML = `
                <tr><td colspan="24" class="px-3 py-2 text-center text-gray-500">
        ...
        tableBody.innerHTML = `
            <tr><td colspan="24" class="px-3 py-2 text-center text-red-500">
