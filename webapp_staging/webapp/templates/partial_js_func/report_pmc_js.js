...
const tableBody = document.getElementById("childrenTableBody");
const submitBtn = document.getElementById("submitbuttonsmc");
editSmcForm.addEventListener("submit", async (e) => {
    ...
    const campaignid = parseInt(formData.get("campaign")); // fallback for testing
    const iduc = parseInt(formData.get("uc")); // fallback for testing
    try {
        // const res = await fetch(`${window.API_BASE_URL}/campaign/read/smc_persistent_child?aic_identifier=${idheader}&campaign_id=${campaignid}&uc_id=${iduc}`, {
        const res = await fetch(`${window.API_BASE_URL}/campaign/read/smc_persistent_child?campaign_id=${campaignid}&uc_id=${iduc}`, {
            method: "POST",
            headers: { "accept": "application/json" }
        });
        ...
        if (json.message.includes("No")) {
            console.log("yes");
            submitBtn.classList.add("hidden");
        }
        if (json.status === "success" && Array.isArray(json.data.child_data)) {
            json.data.child_data.forEach(child => {
                const tr = document.createElement("tr");
                tr.innerHTML = `
                      <td class="px-4 py-3">${child.aic_identifier}</td>
                      <td class="px-4 py-3">${child.aic_name}</td>
                      <td class="px-4 py-3">${child.id_team}</td>
                      <td class="px-4 py-3">${child.username}</td>
                      <td class="px-4 py-3">${child.campaign_name}</td>
                      <td class="px-4 py-3">${child.day_no}</td>
                      <td class="px-4 py-3">${child.house_no}</td>
                      <td class="px-4 py-3">${child.child_name}</td>
                      <td class="px-4 py-3">${child.father_name}</td>
                      <td class="px-4 py-3">${child.gender}</td>
                      <td class="px-4 py-3">${child.age_plus_months_elapsed}</td>
                      <td class="px-4 py-3">${child.no_of_missing_round}</td>
                      <td class="px-4 py-3">${child.address}</td>
                      <td class="px-4 py-3">${child.missed_reason}</td>
                      <td class="px-4 py-3">${child.sub_reason}</td>
                      <td class="px-4 py-3">${child.location}</td>
                      <td class="px-4 py-3">${child.mmp}</td>
                      <td class="px-4 py-3">${child.return_date ?? ""}</td>
                      <td class="px-4 py-3">${child.entered_by}</td>
                      <td class="px-4 py-3">${child.username}</td>
                      ...
                      <td class="px-4 py-3">
                        ...
                        <input type="hidden" name="child[${child.id_child}][aic_identifier]" value="${child.age_plus_months_elapsed}">
                        <input type="hidden" name="child[${child.id_child}][team_id]" value="${child.aic_id}">
                        <input type="hidden" name="child[${child.id_child}][team_id]" value="${child.id_team}">
                        <input
                          type="checkbox"
                          class="report-smc-checkbox w-8 h-8 accent-red-600 rounded border-gray-300 focus:ring-red-500"
                          data-checkboxindexing="${child.id_child}"
                          data-checkboxstylingindex="${child.age_plus_months_elapsed}"
                          data-checkboxstylingid="${child.id_campaign_to_considered}"
                        >
                      </td>`;
...
<script>
document.addEventListener("DOMContentLoaded", () => {
    const form = document.getElementById("editpmcstatus");
    const submitBtn = document.getElementById("submitbuttonsmc");

    form.addEventListener("submit", async (e) => {
        e.preventDefault(); // stop page reload
        const checkedChildren = Array.from(document.querySelectorAll(".report-smc-checkbox:checked"))
            .map(cb => ({
                id_child: cb.dataset.checkboxindexing,
                age: cb.dataset.checkboxstylingindex,
                idcampaign: cb.dataset.checkboxstylingid,
            }));
        // Build payload
        const payload = {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const checkedChildren = Array.from(document.querySelectorAll(".report-smc-checkbox:checked"))
            .map(cb => ({
                idchildren: parseInt(cb.dataset.checkboxindexing, 10),
                age: parseInt(cb.dataset.checkboxstylingindex, 10),
                campaign_id: parseInt(cb.dataset.checkboxstylingid, 10)
            }));

        // Build payload to match backend
        const payload = {
            child_data: checkedChildren.map(c => ({ idchildren: c.idchildren, age: c.age })),
            user_id: parseInt(window.user_info?.idusers),       // you can set this dynamically if needed
            campaign_id: checkedChildren[0].campaign_id    // same here, dynamic if required
        };

        console.log("Submitting payload:", payload);

        try {
            const response = await fetch(`${window.API_BASE_URL}/campaign/add/status/pmc/child`, {
                method: "POST",
                headers: {
